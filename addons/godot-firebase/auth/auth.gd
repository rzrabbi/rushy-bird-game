...
# Link Email/Password credential to existing user (e.g. anonymous upgrade)
func link_with_email_and_password(id_token: String, email : String, password : String) -> void:
	if _is_ready():
		is_busy = true
		var body := {
			"idToken": id_token,
		...
		# We reuse SIGNUP_EP so if it fails, it emits signup_failed
		auth_request_type = Auth_Type.SIGNUP_EP
		request(_base_url + _update_account_request_url, _headers, true, HTTPClient.METHOD_POST, JSON.print(body))
...
# Link OAuth credential to existing user (e.g. anonymous upgrade)
func link_with_oauth(id_token: String, _token: String, provider: AuthProvider) -> void:
	var token : String = _token.percent_decode()
	print(token)
	var is_successful: bool = true
	if provider.should_exchange:
	...
	if is_successful and _is_ready():
		is_busy = true
		var body := _oauth_login_request_body.duplicate()
		body.postBody = "access_token="+token+"&providerId="+provider.provider_id
		body.requestUri = _local_uri
		body.idToken = id_token
		requesting = Requests.LOGIN_WITH_OAUTH
		auth_request_type = Auth_Type.LOGIN_OAUTH
		request(_base_url + _signin_with_oauth_request_url, _headers, true, HTTPClient.METHOD_POST, JSON.print(body))
...
func _on_FirebaseAuth_request_completed(result : int, response_code : int, headers : PoolStringArray, body : PoolByteArray) -> void:
	# Removed print_debug here to avoid console output overflow crashes
	is_busy = false
	var res
	if response_code == 0:
	...
	if response_code == HTTPClient.RESPONSE_OK:
		if not res.has("kind"):
			var clean_res = get_clean_keys(res)
			for key in clean_res.keys():
