...
export var _audioMotorPath = ["res://addons/WebAudioExternal/js/howler.min.js","res://addons/WebAudioExternal/js/howler.spatial.min.js","res://addons/WebAudioExternal/js/audioEngine.js"]
...
func _init():
	if enable:
		print("AudioManager: WebAudioExternal initializing...")
		JavascriptManager.init(_audioMotorPath)
		var res = JavascriptManager._run("var "+js_object_name+" = new AudioEngine();")
		print("AudioManager: Instantiate AudioEngine result: ", res)
...
func play(_audio_name,_config={}):
	if enable and isLoaddedAllAudios:
		...
		var merged_config = {}
		if group_audios_config.has(group_audio):
			merged_config = group_audios_config[group_audio].duplicate()
		for key in _config:
			merged_config[key] = _config[key]
		_config_converted = convert_config(merged_config)
