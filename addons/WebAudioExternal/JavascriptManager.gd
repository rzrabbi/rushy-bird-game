...
func init(compile_files_js):
	for _file in compile_files_js:
		var code = loadFile(_file)
		if code != "":
			print("JavascriptManager: Evaluating JS file: ", _file)
			JavaScript.eval(code, true)
		else:
			printerr("JavascriptManager: Skipped evaluating empty/missing JS file: ", _file)


func loadFile(_file):
	var file = File.new()
	var err = file.open(_file, file.READ)
	if err != OK:
		printerr("JavascriptManager: Failed to open file: ", _file, " Error code: ", err)
		return ""
