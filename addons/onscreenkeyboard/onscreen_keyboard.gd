...
var backspace_timer: Timer = null
...
func _enter_tree():
	...
	backspace_timer.name = "BackspaceTimer"
	backspace_timer.one_shot = true
	add_child(backspace_timer)
	backspace_timer.connect("timeout", self, "_on_backspace_timer_timeout")
	
	# Instantiate Audio Player for key click sounds
	click_sound = AudioStreamPlayer.new()
	if OS.has_feature("HTML5"):
		click_sound.set_script(load("res://scripts/WebAudioPlayer.gd"))
		click_sound.sound_name = "keyboard.wav"
	click_sound.stream = load("res://assets/sounds/keyboard.wav")
	click_sound.set_volume_db(-6.0)
...
func _hideKeyboard(keyData=null):
	...
	if is_instance_valid(backspace_timer):
		backspace_timer.stop()
...
func _keyPressed(keyData):
	if keyData.has("output") and keyData.get("output") == "Backspace":
		is_backspace_held = true
		_delete_char()
		if is_instance_valid(backspace_timer):
			backspace_timer.wait_time = 0.4
			backspace_timer.one_shot = true
			backspace_timer.start()
...
func _on_backspace_timer_timeout():
	if is_backspace_held:
		_delete_char()
		if is_instance_valid(backspace_timer):
			backspace_timer.wait_time = 0.07
			backspace_timer.one_shot = true
			backspace_timer.start()
	...
	for layout in data.get("layouts"):
		...
		for row in layout.get("rows"):
			...
			for key in row.get("keys"):
				...
				newKey.connect("down",self,"_keyPressed")
...
func _update_custom_caret_for_field(field: LineEdit):
	...
	if OS.has_feature("HTML5"):
		is_mobile = JavaScript.eval("/Mobi|Android|Tablet|iPad|iPhone/i.test(navigator.userAgent)")
	if font is DynamicFont and not is_mobile:
